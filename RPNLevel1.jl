...
for i=1:imgs
	...
	for j=1:batchcount
		...
		println("Image: $i Batch: $(convert(Int32,j)) Accuracy: $((1-loss)*100)%");
	end
end
...
@knet function RPNLevel1(x)
	...
	b=par(init=RPNWeights["reduction1_b"], dims=(1,1,1,192))
	m=conv(w,m; window=1, padding=0, stride=1)
	m=relu(m.+b)
	
	
	w=par(init=RPNWeights["conv_cls_score1_w"], dims=(2,2,2,192,8))
	b=par(init=RPNWeights["conv_cls_score1_b"], dims=(1,1,1,8))
	m=conv(w,m; window=2, padding=1, stride=1)
	m=reshape(m; outdims=(2,2,4,53,53,26))	
	 return wbf(m; out=2, f=:soft)
end

function Network(im3d)
    f3d=compile(:RPNLevel1)
	v3d=forw(f3d, im3d)
